...
class LoanCalculator:
    def __init__(self):
        ...
        b1MonthlyPayment = Label(window,textvariable=self.monthlyPaymentVar).grid(row=4,column=2,sticky=E)
        
        self.totalPaymentVar = StringVar()
        b1MountlyPayment = Label(window,textvariable=self.totalPaymentVar).grid(row=5,column=2,sticky=E)
        
        btCP = Button(window,text='Compute Payment',command=self.computePayment).grid(row=6,column=2,sticky=E)
